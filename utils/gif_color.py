...
def process_lammps_data(folder, pattern, file_pattern, w, h, g, border, color1, color2, particles_r):
    ...
    for traj_file in path_trj:
        ...
        #print((c_list, ImageColor.getrgb(color1), ImageColor.getrgb(color2)))
        pipeline.modifiers.append(lambda frame, data: compute_particle_colors(frame, data, c_list, ImageColor.getrgb(color1), ImageColor.getrgb(color2)))
        ...
        for frame in range(pipeline.source.num_frames):
            ...
            process_frame(img, frames, g, border)
...
def process_frame(img, frames, g, border):
    ...
    pixels[non_transparent_mask, :3] = [int(g * 255), int(g * 255), int(g * 255)]  # Set non-transparent pixels to gray

    # Create an image from the modified pixels array
    result = Image.fromarray(pixels, 'RGBA')

    # Apply MaxFilter to expand non-transparent pixels
    result = result.filter(ImageFilter.MaxFilter(border))

    # Composite the result with the original image to retain the original image's transparency layout
    final_img = Image.alpha_composite(result, img)

    # Ensure the final image is in 'RGBA' mode before appending to the frames list
...
def save_gif(frames, path):
    # Prepare frames: ensure palette is consistent and transparency is handled
    c_frames = []
    for frame in frames:
        if frame.mode != 'P':
    ...
    # Save the GIF with a specified transparency index
    c_frames[0].save(
        path, save_all=True, append_images=c_frames[1:], transparency=0,
        disposal=2, loop=0, duration=[10] * (len(c_frames) - 1) + [1000]
    )
