...
def process_lammps_data(folder, pattern, file_pattern, w, h, g, border, e_min, e_max, particles_r):
...
def prepare_frames(frames):
    processed_frames = []
    for frame in frames:
        processed_frame = process_frame(frame, 0.5, 3)
        processed_frame = processed_frame.convert('P', palette=Image.ADAPTIVE, colors=255)
...
def save_gif(frames, path):
    frames[0].save(
        ...
        transparency=0, disposal=2, loop=0, duration=[10] * (len(frames) - 1) + [1000]
    )
